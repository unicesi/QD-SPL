...
class UIControllerTemplate {
	...
	def static generate(String packageName, BusinessEntity authEntity, HashSet<BusinessEntity> bes) '''
...
«««						.get«authEntity.name.toFirstUpper»Services(user.get«DomainCodeUtilities.getID(authEntity).name.toFirstUpper»());
...
«««				«ENDIF»
				...
				«ENDFOR»
				«IF authorizerSelected»
				if (AuthorizerController.getInstance().hasAdminRole(user.get«DomainCodeUtilities.getID(authEntity).name.toFirstUpper»())) {
