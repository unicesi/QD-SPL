...
import co.shift.pcs.authorization.boundary.IAuthorizationManager;
...
public class AuthorizerController implements ProcessContributor {
	...
	public List<ServiceTO> getUserServices(String user) {
		return authManager.getUserServices(user);
	}
	
	public void authorize(ServiceTO service) throws Exception {
		UserTO to = Registry.get(PCSUI.USER);
		List<ServiceTO> userServices = authManager.getUserServices(to.getCc());
	...
	private void listAllUsers() {
		List<UserTO> users = UserController.getInstance().getAllUsers();
		AuthorizerForm aF = Registry.get(AuthorizerForm.ID);
		aF.getTableUsers().removeAllItems();
		for (UserTO userTO : users) {
			Object data[] = { userTO.getCc(), userTO.getName() };
	...
	public void listUserRoles(String user) {
		List<RoleTO> roles = authManager.getUserRoles(user);
		AuthorizerForm aF = Registry.get(AuthorizerForm.ID);
	...
	public boolean addRoleToUser(String user) {
		PCSUI ui = Registry.get(PCSUI.UI);
		try {
			PopUpUserRoleForm uRF = Registry.get(PopUpUserRoleForm.ID);
			RoleTO role = (RoleTO) uRF.getComboBox_1().getValue();
			if (role != null) {
				if (authManager.addRoleToUser(user, role)) {
					ui.showNotification(PCSUI.MSG_NOT,
	...
	public boolean hasAdminRole(String user) {
		List<RoleTO> userRoles = authManager.getUserRoles(user);
