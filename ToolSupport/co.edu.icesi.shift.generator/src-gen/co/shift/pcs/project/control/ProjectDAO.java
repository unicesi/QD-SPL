...
public class ProjectDAO implements IProjectDAO {
	...
	public List<ProjectTO> getProjectFromUser(String userCc){
		...
		Query query;
		List<Object[]> foundProjects;
		try {
			query = em
					.createNativeQuery("SELECT u.* FROM Project u, UserProject pu WHERE u.id = pu.projectId AND pu.userCc = '"
							+ userCc+"'");
			foundProjects = query.getResultList();
		} catch (Exception e) {
			query = em
					.createNativeQuery("SELECT u.* FROM Project u, ProjectUser pu WHERE u.id = pu.projectId AND pu.userCc = '"
							+ userCc+"'");
			foundProjects = query.getResultList();
		}
		ProjectTO u = null;
