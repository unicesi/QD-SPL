	...
	override doGenerate(Resource input, IFileSystemAccess fsa) {
		...
		for (k : input.allContents.toIterable) {
			if (k instanceof Business) {
				val b = k as Business
				appName = b.name
				associations = getBusinessAssociations(b)
			}
			if (k instanceof BusinessEntity) {
				...
				if (getDetailMultipleAssociations(be, associations).size > 0 && !isDetail(be)) {
					fsa.generateFile(
		...
		extendContribution("_r_2_11", CONTRIBUTE_TO_GENERATION, fsa, appName, authEntity);
		...
		fsa.generateFile(
						...
						UIControllerTemplate::generate(appName, authEntity, businessEntities))
		fsa.generateFile(
